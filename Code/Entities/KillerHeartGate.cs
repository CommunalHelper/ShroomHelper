...
namespace Celeste.Mod.ShroomHelper.Entities {
    ...
    public class KillerHeartGate : Entity {
        ...
        private readonly bool startHidden = true;
        ...
        public override void Added(Scene scene) {
            ...
            level.Add(TopSolid = new Solid(new Vector2(X, level.Bounds.Top - 32), Size, Y - level.Bounds.Top + 32f, safe: true));
            TopSolid.SurfaceSoundIndex = 32;
            TopSolid.SquishEvenInAssistMode = true;
            TopSolid.EnableAssistModeChecks = false;
            level.Add(BotSolid = new Solid(new Vector2(X, Y), Size, level.Bounds.Bottom - Y + 32f, safe: true));
            BotSolid.SurfaceSoundIndex = 32;
            BotSolid.SquishEvenInAssistMode = true;
            BotSolid.EnableAssistModeChecks = false;
            if (SceneAs<Level>().Session.GetFlag("opened_heartgem_door_" + Requires)) {
                Opened = true;
                Visible = true;
                Counter = Requires;
        ...
        private void DrawInterior(Rectangle bounds) {
            ...
            Vector2 value = SceneAs<Level>().Camera.Position;
            if (Opened) {
                value = Vector2.Zero;
            }
            ...
            public override void Update() {
                ...
                for (float num = (int)level.Camera.Left; num < level.Camera.Right; num += 1f) {
                    if (num < X || num >= X + blockSize) {
                        level.Particles.Emit(P_Slice, new Vector2(num, Y));
                    }
                }
            }
