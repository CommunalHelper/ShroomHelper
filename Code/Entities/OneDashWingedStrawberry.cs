...
namespace Celeste.Mod.ShroomHelper.Entities
{
    ...
    class OneDashWingedStrawberry : Strawberry
    {
        protected DynData<Strawberry> baseData;

        bool despawnFromSessionIfDashedTwiceInSameRoom;

        public OneDashWingedStrawberry(EntityData data, Vector2 offset, EntityID gid)
            : base(data, offset, gid)
        {
            baseData = new DynData<Strawberry>(this);
            baseData.Set<bool>("Winged", true);
            baseData.Set<bool>("Golden", true);
        ...
        private void OnDash(Vector2 dir)
        {
            ...
            {
                if (!baseData.Get<bool>("flyingAway") && !WaitingOnSeeds && !(Follower.Leader?.Entity is Player))
                {
                    base.Depth = -1000000;
                    Add(new Coroutine(FlyAwayRoutine()));
                    baseData.Set<bool>("flyingAway", true);
        ...
        private IEnumerator FlyAwayRoutine()
        {
            ...
            baseData.Set<float>("flapSpeed", -200f);
            Tween tween = Tween.Create(Tween.TweenMode.Oneshot, Ease.CubeOut, 0.25f, start: true);
            tween.OnUpdate = delegate (Tween t)
            {
                baseData.Set<float>("flapSpeed", MathHelper.Lerp(-200f, 0f, t.Eased));
            };
            Add(tween);
            yield return 0.1f;
            ...
            tween.OnUpdate = delegate (Tween t)
            {
                baseData.Set<float>("flapSpeed", MathHelper.Lerp(0f, -200f, t.Eased));
            };
            Add(tween);
        }
